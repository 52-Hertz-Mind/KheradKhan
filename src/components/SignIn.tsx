        ...
        <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50' onClick={closePopup}>
            <div
                ...
                <form onSubmit={signIn} className='flex flex-col gap-5' dir='rtl'>
                    ...
                    <input required type='email' placeholder='ایمیل' className='p-2 border-2 rounded-2xl' value={email} onChange={(e) => setEmail(e.target.value)}/>
                    <label htmlFor='password'>گذرواژه</label>
                    <input required type='password' placeholder='گذرواژه' className='p-2 border-2 rounded-2xl' value={password} onChange={(e) => setPassword(e.target.value)}/>
                    <button onSubmit={signIn}
